...
Component({
    ...
    methods: {
        _Click: function _Click(e) {
            if (this.data._pureFormItems) {
                this.data._pureFormItems.forEach(function (item) {
                    if (item.type === 'radio' || item.type === 'checkbox' || item.type === 'switch') {
                        item.target._CheckControll(e);
                        var buttonInFormItem = item.target.selectComponent('._mui-inner-button');
                        if (buttonInFormItem) {
    ...
    observers: Object.assign({ 'checked, disabled, onChange, value, _pureFormItems': function checkedDisabledOnChangeValue_pureFormItems() {
            if (this._ArrangeFormItems) {
